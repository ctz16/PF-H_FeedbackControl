...
//coefficients of fitting (X_T*X)^-1*X_T, X= z position
const double c[3][7] = {
...
//attention: to use volatile for variables used in interrupt
...
void setup()
{
  ...
  // clock = clk_io/8 = 16MHz/8 = 2MHz, fast PWM mode, set at match and clear at bottom
  // T = 128 microseconds
  TCCR3A = _BV(COM3A1) | _BV(COM3A0) | _BV(COM3B1) | _BV(COM3B0) | _BV(COM3C0) | _BV(COM3C1) | _BV(WGM30);
  TCCR3B = _BV(WGM32) | _BV(CS31);

  // 255 is no signal at 0V, 0 is 5V, but with delta signals to 0V
  // because a compare match at 0, but match ignored at 255
  D_PF = 255;
  D_H2 = 255;
  D_H4 = 255;

  //timer5 interrupt to control PF preprogrammed waveform
  //CTC mode, clock = clk_io/8 = 2MHz
  bitClear(TIMSK5, OCIE5A);
  TCCR5A = 0;
...
void loop()
{
  ...
  if (triggerflag)
  {
    ...
    //cutoff H wave when PF3 wave ends
     ...
     * psi[0] is flux loop
     * psi[1] to psi[6] are saddle loops
     */
      ...
      if (duty_z > 0)
      {
        if (lastDuty_z < 0)
        {
          D_H2 = 255;
          digitalWrite(H3, LOW);
          // delay one period to make sure no short circuit
      ...
      /********** stop with time limit and op signal limit **********/
  ...
  else
  {
    ...
    /********** send commands and coefficients ***********/
    if (Serial3.available())
    {
      ...
      case 'H':
        val = Serial3.parseFloat();
        tfoff_o = val;
        Serial3.println("tfoff_o set!");
        Serial3.println(tfoff_o,10);
        break;
      case 'I':
        val = Serial3.parseFloat();
        c_bt = val;
